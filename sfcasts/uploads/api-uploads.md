...
upload article reference. This is a valid way to make a upload end point and let me
show you the exact what I mean here. I'm going to use a post man behind the scenes to
...
copy are you are all here and we'll change that. /admin such articles has 51
/references is are you were all to our um, upload end point and we're going to make
...
file. So I can actually choose a file here, which is our earth, that jpeg. And that's
it.
...
login just for testing things temporarily. Let's actually take off the at is granted
...
like this. But there's another way to do it. And if you're used to building a pis
...
make a request, we would send some data that looked like this file name

= space, that text. And maybe there's a whole bunch of other fields. Normally Api is
...
quick, I'm a search for base 64 in code online just to find some site that can base
...
statement here says if requests headers,->get content type = equals, = apple
occasion. /JSON, then we're going to do one thing else. We're gonna do our norm,
normal code. And the only thing that's really gonna differ is this uploaded file part
...
for this. I'm a de serialize the object. Now earlier we use the DC relaxer. If you
search for DC, realize inside of here and our update thing, we actually sent that
JSON Body that looked like our, um, our article reference object. It only had one
field, but basically match the fields of the Arctic reference object. And we do see
realized directly into our article reference a entity. In this case though, if you
look at our fields here, there is a file name field on our, uh, article entity. Um,
...
directory and inside of there and new class, how about Article Reference Upload Api
model. This is going to be a very simple class and we're just going to create a
public file name property and a public data property. Yes, I'm getting public because
...
controller, I'll add a new argument down here, which is going to be serialized or
interface serializer. And then we can say upload Api model = and we use the DC
realize method, which takes three arguments. The first one is the actual JSON. So
that's going to be request->get content. The second thing is the type of object to it
should be turned into, which is an Article Reference Upload Api model, ::class. And
the third option is JSON, the format, we don't need a context. In this case, we're
just going to, because we're not DC realizing into an existing object and we don't
need to use any API groups and the Walmart here because we have added some, uh, and
um, validation rules to this. Let's go ahead and just check for, um, the validators
from if they violations = validator->validate the upload API model. If Violations
Aero count.

Yeah.

Oh, sure. That's violations. And if violet violent violations,->count, that's greater
than zero, we're going to return the normal, this Arab JSON Violations 400. Then down
at the bottom, let's just dd the upload an APM owl so we can see if this guy's
...
you can create a public function. Set. Data we'll have is taken a normal string in
case the user forgot that field. And inside we'll say this->data = data. And I'm also
going to create a, another private decoded at data and will say this->decoded data,
people's base 64 d code data. And because this is private and doesn't have a setter,
...
Ultimately, the else part of our thing has an uploaded file object. If we can create
...
we can't actually create an upload file object. That's something that's, that's
unique. Um, it has some unique properties of true PHP file uploads, but we can create
a file object as reminder of what I'm talking about in the article fixture at the
bottom, we actually created a new file object that's the parent class of the upload a
file object. And we pass it a target path, which was actually a file on the
filesystem. So we can do the exact same thing here. So check this out. First, let's
get a temp path set to CIS. Get tempter and obviously SF_upload that unique id, so
...
so first I need to get, be able to get the Dakota data saw, add public function, get
coded data, which is going to be a nullable string. That's a return decoded data. And
here we'll say file, put contents 10 path and then uploaded API model era. Get Dakota
data, Oh and exam not getting position on that. That's because seriously, we need to
...
and now we can finally say uploaded file, same variable name. His reasoning down here
= new file. This time we want the one from http foundation, but when you hit tab,
...
about file object? I know a little bit, a little bit ugly. So down here we can say
new file object and we'll pass it the temporary path. And now let's dd the uploaded
...
method. Get client original name on our file object. And this is coming down here on
[inaudible] 84 I of course. So the uploaded file object actually has a few methods of
file object doesn't most notably gets client original name is something that only
lives on the uploaded file object so that's easy to fix. We can just go up here. I'm
going to create an original name variable on both sides here. I'll send it to
uploaded API model aero file name cause you don't have the original name to be
whatever the user is sending in our request. And down here I'll say original original
name = we'll say uploaded file Arrow, get client original name. We copied that
original name and we'll go down here to set the original name and we'll just use that
and if it doesn't exist for some reason we'll use file name. All right, let's see if
...
back over to our web interface. They're their space dot text. We can download that.
And over here in s three, it is actually available
...
tried to like read the mime type from the file, I'm going to say if his file uploaded
file->get path name, then we're going to delete it. And I'm checking the if statement
here because if this was an actual uploaded file, then when we, then the, um, it may
already be gone. I don't know. Unlink uploaded file, Arrow, get path name and just to
see if this is working. Let's DD upload a file->get path name, we'll go back over the
...
celebrate, remove that deedee.

I'm going to hit send them one more time and it works perfectly. Now that we know
that, don't forget to go back and add your security here at is granted manage four
